<template>
    <div class="settings__container">
        <header>
            <a href="https://github.com/iqtek/asterisk_ng" target="”_blank”">
                <AsteriskLogo class="settings__logo" />
            </a>
            <span v-bind:style="statusStyle" class="current__status">{{
                currentStatus
            }}</span>
        </header>
        ...
        <div class="settings__container__main">
            ...
            <div class="settings__input__container">
                ...
                <label for="enableNotification">
                    {{
                        _i18n(
                            "domain.amocrm.settings.field.enable_notification"
                        )
                    }}
                </label>
            </div>
        ...
        <div class="settings__save__container">
            ...
            <div
                class="save__message__container"
                v-if="!isDisabled && saveMessage.length > 0"
            >
                ...
                <span
                    class="save__message"
                    v-bind:class="{ save__message__hidden: isChecked }"
                >
                    {{ saveMessage }}
                </span>
            </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        _getPingFunction(
            ...
            const serverFactory: ServerFactory =
                Container.getValue("ServerFactory");
            const server: IServer = serverFactory.getInstance(
                serverAddress,
                secretKey
            );
        ...
        onChange: debounce(function () {
            ...
            promise.then(
                (result: void) => {
                    this.saveMessage = this._i18n(
                        "domain.amocrm.settings.status.connected"
                    );
                ...
                (error: ServerError) => {
                    if (error instanceof InvalidAuthenticationData) {
                        this.saveMessage = this._i18n(
                            "domain.amocrm.settings.status.invalid_secret_key"
                        );
                        this.checkStatus = CheckStatus.ERORR;
                    } else if (error instanceof IncompatibleVersionError) {
                        this.saveMessage = this._i18n(
                            "domain.amocrm.settings.status.incompatible_version"
                        );
                        this.checkStatus = CheckStatus.ERORR;
                    } else if (error instanceof ServerNotResponsingError) {
                        this.saveMessage = this._i18n(
                            "domain.amocrm.settings.status.not_available"
                        );
                        this.checkStatus = CheckStatus.CRITICAl;
                    } else {
                        this.saveMessage = this._i18n(
                            "domain.amocrm.settings.status.unknown_error"
                        );
                        this.checkStatus = CheckStatus.CRITICAl;
                    }
                    this.isChecked = false;
        ...
        getStatus() {
            const pingFunction: IPingServerFunction = Container.getValue(
                "IPingServerFunction"
            );
    ...
    computed: {
        isDisabled() {
            return (
                JSON.stringify(this.settingsModel) === this.initialSettingsModel
            );
        },
    },
    mounted() {
        ...
        this.settingsInput = rootElement.querySelector(
            `input[name=${inputName}]`
        );
        this.saveButton =
            rootElement.getElementsByClassName("js-widget-save")[0];
        ...
        function setDefaultSettings() {
            ...
            self.currentStatus = self._i18n(
                "domain.amocrm.settings.status.not_configured"
            );
        }
